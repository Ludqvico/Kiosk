...
DWORD WINAPI LAST2(LPVOID) {
  ...
  CloseHandle(hObject);
  CloseHandle(v4);
  // Sleep(0xFFFFFFFF); // Removed indefinite sleep
  return 0;
}
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  // Basic GDI Prank Sequence
  // This wrapper ensures the prank runs for a limited time and cleans up.

  // Seed random
  srand(GetTickCount());

  // Hide Console if present (though this is WinMain)
  ...
  // Terminate initial threads to switch phases
  TerminateThread(hThread1, 0);
  TerminateThread(hThread2, 0);

  // Phase 2: Intense
  LAST2(
      NULL); // This function runs move, rotate, whiteness for 30s then returns
