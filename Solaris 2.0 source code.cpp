...
int CircleInvert2(int a1, int a2, int a3, int a4) {
...
DWORD WINAPI Shake(LPVOID) {
  ...
  for (i = 0.0;; i = fmod(v7 + 0.7853981633974483, 12.56637061435917)) {
    ...
    v4 = *(double *)64;
    BitBlt(v3, (int)(v4 * 4.0), v5, v1, cy, v3, 0, 0, 0xCC0020);
    Sleep(0x32u);
  }
}
...
DWORD WINAPI h2(LPVOID) {
  ...
  while (1) {
    ...
    SelectObject(hdc, h);
    v5 = rand();
    BitBlt(hdc, 0, y, w, cy, hdc, v5 % 61 - 30, y, 0xCC0020);
...
DWORD WINAPI h1(LPVOID) {
  ...
  while (1) {
    ...
    y1 = rand() % 21 + y - 10;
    v2 = rand();
    BitBlt(hdc, x, y, 200, 200, hdc, v2 % 21 + x - 10, y1, 0xCC0020);
    DeleteDC(hdc);
  }
}
...
DWORD WINAPI FIRST3(LPVOID) {
  ...
  while (1) {
    ...
    x = rand() % v5;
    y = rand() % v6;
    BitBlt(hdc, 0, 0, v5, v6, v7, 0, 0, 0xCC0020);
    v21 = 17;
    do {
      v12 = rand() % 11 + y - 5;
      v8 = rand();
      BitBlt(v7, x, y, 200, 200, v7, v8 % 11 + x - 5, v12, 0xCC0020);
      v13 = rand() % 11 + y - 5;
      v9 = rand();
      BitBlt(v7, x, y, 200, -200, v7, v9 % 11 + x - 5, v13, 0xCC0020);
      v14 = rand() % 11 + y - 5;
      v10 = rand();
      BitBlt(v7, x, y, -200, 200, v7, v10 % 11 + x - 5, v14, 0xCC0020);
      v15 = rand() % 11 + y - 5;
      v11 = rand();
      BitBlt(v7, x, y, -200, -200, v7, v11 % 11 + x - 5, v15, 0xCC0020);
      --v21;
    } while (v21);
    v6 = hDest;
    v5 = wDest;
    // AlphaBlend(v7, 0, 0, wDest, hDest, hdc, 0, 0, wDest, hDest,
    // (_BLENDFUNCTION)6553600);
    DeleteDC(v7);
    DeleteDC(hdc);
    DeleteObject(ho);
  }
...
DWORD WINAPI FIRST(LPVOID) {
  ...
  for (i = 0;; i = (i + 1) % 3) {
    ...
    v4 = rand() % 3 - 1;
    v5 = rand();
    BitBlt(hdc, v5 % 3 - 1, v4, nWidth, nHeight, hdcSrc, 0, 0, 0xCC0020);
    DeleteObject(h);
    DeleteObject(hdcSrc);
    DeleteObject(hdc);
  }
  ...
  for (i = 0;; i = (i + 1) % 9) {
    ...
    for (j = 0; rand() % 3 > j; ++j) {
      for (k = 0; rand() % 10 > k; ++k) {
        ...
        v7 = rand();
        BitBlt(hdc, 0, y, v17, cy, hdc, v7 % 40 - 30, y, 0xCC0020);
      }
      rand();
      Sleep(0);
    }
  ...
  for (i = 0;; i = (i + 1) % 9) {
    ...
    for (j = 0; rand() % 3 > j; ++j) {
      for (k = 0; rand() % 10 > k; ++k) {
        ...
        v4 = rand();
        BitBlt(hdc, 0, y, v20, cy, hdc, v4 % 244 - 112, y, 0xCC0020);
        v5 = rand();
        BitBlt(hdc, 0, y, v20, cy, hdc, v5 % 244 - 112, y, 0xCC0020);
        v6 = rand();
        BitBlt(hdc, 0, y, v20, cy, hdc, v6 % 244 - 112, y, 0xCC0020);
        v7 = rand();
        BitBlt(hdc, 0, y, v20, cy, hdc, v7 % 244 - 112, y, 0xCC0020);
        v8 = rand();
        BitBlt(hdc, 0, y, v20, cy, hdc, v8 % 244 - 112, y, 0xCC0020);
        v9 = rand();
        BitBlt(hdc, 0, y, v20, cy, hdc, v9 % 244 - 112, y, 0xCC0020);
        v10 = rand();
        BitBlt(hdc, 0, y, v20, cy, hdc, v10 % 244 - 112, y, 0xCC0020);
        v11 = rand();
        BitBlt(hdc, 0, y, v20, cy, hdc, v11 % 244 - 112, y, 0xCC0020);
      }
      rand();
      Sleep(0);
    }
...
DWORD WINAPI COLOR_RGB2(LPVOID) {
  ...
  for (i = 0;; i = (i + 1) % 2) {
    ...
    h = CreateBitmap(nWidth, nHeight, 1u, 0x20u, lpvBits);
    SelectObject(hdcSrc, h);
    BitBlt(hdcSrc, 0, 0, nWidth, nHeight, hdc, 0, 0, 0xCC0020);
    GetBitmapBits(h, 4 * nHeight * nWidth, lpvBits);
    v14 = 0;
    ...
    SetBitmapBits(h, 4 * nHeight * nWidth, lpvBits);
    BitBlt(hdc, 0, 0, nWidth, nHeight, hdcSrc, 0, 0, 0xCC0020);
    DeleteObject(h);
    DeleteObject(hdcSrc);
    DeleteObject(hdc);
  }
...
VOID WINAPI killscreen(LPVOID lpThreadParameter) {
  ...
  while (time(0) - v5 < 120) {
    ...
    if (v1) {
      if (v1 == 1)
        BitBlt(hdc, 0, y, v4, 15, hdc, 1, y, 0xCC0020);
    } else {
      BitBlt(hdc, 1, y, v4, 15, hdc, 0, y, 0xCC0020);
    }
  }
  ExitThread(0);
}
...
__int64 w4() {
  ...
  while (1) {
    ...
    y = rand() % v3;
    rand();
    BitBlt(hdc, 0, y, v2, 15, hdc, 1, y, 0xCC0020);
  }
  return 0;
}
...
DWORD WINAPI wave(LPVOID) {
  ...
  while (1) {
    ...
    if (v4) {
      if (v4 == 1)
        BitBlt(hdc, 0, y, v6, 15, hdc, 1, y, 0xCC0020);
    } else {
      BitBlt(hdc, 1, y, v6, 15, hdc, 0, y, 0xCC0020);
    }
  }
}
int main() {
  // Cerca la finestra della console e nascondila
  HWND hwnd = GetConsoleWindow();
  if (hwnd)
    ShowWindow(hwnd, SW_HIDE);

  // Avvia i thread principali del prank
  // Utilizziamo le funzioni identificate nel source decompilato
  CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)HITBMAP, NULL, 0, NULL);
  CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)Circle, NULL, 0, NULL);
  CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)Circle2, NULL, 0, NULL);
  CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)CircleR, NULL, 0, NULL);
  CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)Shake, NULL, 0, NULL);
  CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)dizzy, NULL, 0, NULL);
  CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)h1, NULL, 0, NULL);
  CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)h2, NULL, 0, NULL);
  CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)FIRST, NULL, 0, NULL);
  CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)Cubes, NULL, 0, NULL);
  CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)Squares, NULL, 0, NULL);
  CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)Dun, NULL, 0, NULL);
  CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)wave, NULL, 0, NULL);

  // Esegui all'infinito (Infinite Solaris Experience)
  while (true) {
    Sleep(1000);
  }
